...
class CompleteTicketService
{
    ...
    {
        Gate::forUser($actor)->authorize('complete', $ticket);
        ...
        return DB::transaction(function () use ($actor, $ticket, $desk): Ticket {
            ...
            if (
                ...
                || $locked->status !== TicketStatus::IN_SERVICE
                || ! $locked->status->canTransitionTo(TicketStatus::COMPLETED)
            ) {
                throw ValidationException::withMessages([
                    'ticket' => 'Só é possível finalizar uma senha em atendimento nesta mesa.',
                ]);
            }
